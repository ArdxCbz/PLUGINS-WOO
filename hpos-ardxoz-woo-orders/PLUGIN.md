...
inline del método de envío); todo lo lee vía `$order->get_meta()` — cero `get_post_meta()` sobre pedidos.
...
| `includes/class-order-column.php` | Reemplaza `order_number` por `haw_order`: nº de pedido (link solo admin/shop_manager), fecha localizada ("Vie, 20 Mar 2026"), hora y "Por: <cliente>". Oculta columnas ruidosas a `vendedor`. |
| `includes/class-info-column.php` | Columna `order_info`: método de envío con badge de color, guía (fallback a `shipping_postcode`), costo de envío, forma de pago. Incluye **modal + AJAX** para cambiar el método de envío (solo admin). |
...
El plugin **solo lee** (nunca escribe metas de depósito/envío; eso lo hace [[plugin-hpos-ardxoz-woo-metaorder]]).
...
- `admin_footer` → modal + JS de cambio de método de envío (solo en pantalla `woocommerce_page_wc-orders`, solo admin).
- `wp_ajax_hawo_cambiar_metodo_envio` → guarda el nuevo método de envío en el item de shipping.
...
| Nonce | Acción |
|---|---|
| `hawo_cambiar_envio` (campo `security`) | AJAX `hawo_cambiar_metodo_envio` |

**Capabilities:** cambiar método de envío y ver `order_payment` exigen `administrator`; `customer_data` exige
...
3. **Guía con doble fallback** — `order_info` busca `numero_guia` vía `Meta_Resolver` (HPOS→ACF) y, si sigue vacío, cae a `shipping_postcode` — porque [[plugin-hpos-ardxoz-woo-demv]] autorrellena el postcode con la guía interna en envíos CBS/LOCAL/SUECIA.
...
7. **Cambio de método de envío HPOS-safe** — El AJAX edita el `method_title` de los items de shipping (o crea uno nuevo si no existe) y hace `$order->save()`. El `<select>` del modal está **hardcodeado** a IBEX/CBS/SUECIA/LOCAL/ENCOMIENDA.
...
9. **Plugin de solo lectura** — Salvo el cambio de método de envío, no escribe ninguna meta de depósito/retorno/envío. Esos datos los produce metaorder/DEMV/actions; aquí solo se visualizan.
...
- **Cuelga de `current_user_can('administrator')`** — Varias columnas chequean el rol `administrator` directamente en vez de una capability (`manage_woocommerce`). Un shop_manager no ve `order_payment` ni puede cambiar el método de envío aunque administre la tienda.
...
- **Select de envío hardcodeado** — Los métodos del modal no se leen de las zonas de envío de WC; si se agrega un método nuevo, hay que editar el HTML.
