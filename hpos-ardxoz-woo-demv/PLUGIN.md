...
| `_hpos_ardxoz_woo_monto_prepago_directo` | — | Monto pagado **directo** por el cliente (full, sin retención IBEX), acumulado al marcar "Pago directo" en Depósitos Express. En IBEX, la retención 7% cae solo sobre `total − prepago` (regla 25). `Calculator::META_PREPAGO_DIRECTO` / `prepago_directo()`. |
...
25. **IBEX con pago directo del cliente (prepago)** — La retención 7% IBEX asume que **todo** el pedido se cobra por COD vía el courier (que retiene el 7% y deposita el neto). Si el cliente paga una parte **directo** (no COD), esa parte entra al banco **full** (sin retención) y el 7% cae solo sobre lo que cobra el courier. Punto único: meta `_hpos_ardxoz_woo_monto_prepago_directo` (`Calculator::prepago_directo()`). `calcular()` IBEX = `prepago + (total − prepago)×0,93`; `sum_ibex_cod_retention()` retiene `(total − prepago)×7%` (LEFT JOIN a `wc_orders_meta`, afecta el tile de stats y el Estado de Resultados de Finanzas). El prepago se captura con el **checkbox "Pago directo del cliente"** en el modal de **Depósitos Express** (`ajax_complete` lo acumula en la meta **antes** de evaluar el gate/plan, de a **un** pedido); sin marca, comportamiento IBEX normal intacto. El depósito directo se registra como su prepago y el pedido **no completa** hasta cubrir `calcular()` (el resto llega como remesa COD, que sí trae el 7% descontado). La página principal de Depósitos (Admin) **no** tiene el checkbox: los pagos directos se registran por Express. **Idempotencia anti doble-submit:** tanto Express (`ajax_complete`) como Admin (`Ajax::complete_deposit`) omiten el pedido si el **comprobante ya está aplicado** a su `numero_deposito` (estilo Caja regla 11) — evita que un reenvío duplicado sume el depósito/adelanto dos veces. El JS de Express además tiene guarda de re-entrada (`submitting`) y re-bindea el botón Guardar con `off().on()` en cada apertura del modal (anula doble binding por doble carga del script). El `$.ajax` del guardado fija `timeout: 60000` y el `.fail()` ya **no** muestra un genérico "Error de red": diagnostica por `jqXHR.status` (0 = sin conexión/timeout/abortado · 403 = sesión/nonce o WAF · 5xx = error del servidor) y loguea status+`responseText` en consola — para distinguir firewall/red de nonce de fatal. En todos los casos aclara que **el depósito NO se registró** (el server rechaza antes de procesar) y deja reintentar.
