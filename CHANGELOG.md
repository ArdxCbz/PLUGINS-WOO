...
- **feat(finanzas-ventova):** La **paginación** del *Historial de movimientos* estrena el diseño de **Depósitos Express** (v2.15): botones redondeados centrados (`«` / números / `»`) con la página activa en azul, en vez del `.tablenav` gris de WP. Sigue siendo server-side (recarga por GET preservando filtros/orden). Nuevo bloque `.fin-pagination` en `admin.css`.
- **feat(inventory-csv-ventova):** El pie **«Σ Gastos»** de una compra ahora muestra el **total gastado en Bs, en $ y el TC promedio** de **TODOS** los registros —facturados **o no** (v3.51)—, no solo los facturados agrupados por moneda: es cuánto se lleva gastado/comprometido en la importación. `expense_grand_totals()` reemplaza a `totals_by_currency()`/`tc_weighted_average()`.
